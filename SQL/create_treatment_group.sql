...
DROP TABLE IF EXISTS scratch.treated_ca_permits CASCADE;
CREATE TABLE scratch.treated_ca_permits
    AS
...
, ciwqs_perms AS (
  ...
  -- Label CA permits by whether they have every been categorized as a POTW
, potws_pre AS (
    ...
    FROM icis.permits 
    WHERE permit_state = 'CA')
  
  -- All POTWs in CA
, potws AS (
  SELECT npdes_permit_id
  FROM potws_pre 
  WHERE rn=1 AND facility_type_indicator = 'POTW'
  )

    -- Select the union of POTWs listed in the treatment regions on CIWQS 
...
INNER JOIN potws USING(npdes_permit_id)
;
COMMIT 
;

SELECT * FROM scratch.treated_ca_permits;
